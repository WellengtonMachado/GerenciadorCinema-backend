...
namespace GerenciadorCinema.Servico.ModuloAutenticacao
{
    public class ServicoAutenticacao : ServicoBase<Usuario, ValidadorUsuario>
    {
        ...
        public async Task<Result<Usuario>> RegistrarUsuario(Usuario usuario, string senha)
        {
            ...
            try
            {
                ...
                Log.Logger.Information("O usuario {UsuarioId} foi registrado com sucesso", usuario.Id);
            }
            catch (Exception ex)
            {
                string msgErro = "Falha no sistema ao registrar o usuário";
        ...
        {
            Log.Logger.Debug("Tentando autenticar o usuário {@u}", email);
        ...
        public async Task<Result<Usuario>> Sair(string email)
        {
            ...
            Log.Logger.Debug("A sessão do usuário {@email} foi removida.", email);
